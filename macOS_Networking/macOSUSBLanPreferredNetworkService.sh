...
# Set USB 10/100/1000 LAN as the highest priority network service.
#
ScriptName="macOSUSBLanPreferredNetworkService.sh"
#
################### Log File Parameters ###################
#
...
#
# Set USB 10/100/1000 LAN as the highest priority network service
function SetUSBLanPriorityNetworkService
	{
		echo "networksetup -ordernetworkservices "USB 10/100/1000 LAN" `networksetup -listallnetworkservices | grep -v 'An asterisk ' |  sed s/\^'*'// | grep -v USB 10/100/1000 LAN | sed 's/.*/\"&\"/' | tr '\n' ' '` "| bash
	}
################### Script ###################
LocalScriptLoggingEnabled
ListNetworkServicePriority
SetUSBLanPriorityNetworkService
ListNetworkServicePriority
